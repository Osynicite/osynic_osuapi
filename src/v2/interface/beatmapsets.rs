...
pub trait IBeatmapsets {
    fn search(&self, params: BeatmapsetsSearchParams) -> impl std::future::Future<Output = Result<BeatmapsetsSearchResponse>> + Send;
    // fn lookup(&self) -> impl std::future::Future<Output = Result<()>> + Send;
    fn get_beatmapset(&self,beatmapset_id: u32) -> impl std::future::Future<Output = Result<Beatmapset>> + Send;
